        ...
        <div className="space-y-6" id="faq-list">
          {FAQ_DATA.map((item, idx) => (
            <div key={idx} className="bg-slate-900 border border-slate-850 rounded-xl p-6 space-y-2">
              <h2 className="text-base sm:text-lg font-serif text-slate-100 font-medium tracking-wide flex items-start space-x-2">
                <HelpCircle className="h-5 w-5 text-amber-500 shrink-0 mt-0.5" />
                <span>{item.question}</span>
              </h2>
              <p className="text-slate-400 text-xs sm:text-sm leading-relaxed pl-7">{item.answer}</p>
            </div>
          ))}
        </div>
