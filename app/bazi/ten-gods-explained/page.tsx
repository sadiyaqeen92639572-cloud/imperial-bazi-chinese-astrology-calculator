              ...
              <div key={tg.code} className="bg-slate-900 border border-slate-850 rounded-2xl p-6 sm:p-8 space-y-4 shadow-md flex flex-col justify-between">
                <div>
                  <div className="flex justify-between items-start">
                    <span className="text-[10px] font-mono font-bold bg-amber-500/10 text-amber-500 px-2 py-0.5 rounded uppercase">
                      {tg.type} God
                    </span>
                    <span className="text-xs text-slate-500 font-mono">Code: {tg.code}</span>
                  </div>
                  ...
                  <h2 className="text-xl font-serif text-slate-100 font-medium tracking-wide mt-2 flex items-center space-x-2">
                    ...
                    <span>{tg.name}</span>
                    <span className="text-amber-500/60 font-serif font-bold text-sm ml-1">({tg.chinese})</span>
                  </h2>

                  <p className="text-xs text-slate-400 mt-2 leading-relaxed bg-slate-950 p-3 rounded-lg border border-slate-850">
                    {tg.summary}
                  </p>
                  ...
                  <div className="space-y-2 mt-4 text-xs text-slate-300">
                    <div>
                      ...
                      <p className="text-slate-400 text-[11px] mt-0.5">{tg.personality}</p>
                    </div>
                    <div>
                      <strong className="text-slate-400">Career &amp; Growth:</strong>
                      <p className="text-slate-400 text-[11px] mt-0.5">{tg.career}</p>
                    </div>
                    <div>
                      <strong className="text-slate-400">Relationship Style:</strong>
                      <p className="text-slate-400 text-[11px] mt-0.5">{tg.relationships}</p>
                    </div>
                  </div>
                </div>
              </div>
