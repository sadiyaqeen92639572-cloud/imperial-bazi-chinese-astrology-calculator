      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-12" id="five-elements-chart-page">
            ...
            >
              <div>
                ...
                <div className="flex flex-wrap gap-1.5 mt-4">
                  {el.traits.map(t => (
                    <span key={t} className="bg-slate-950 text-slate-300 text-[10px] sm:text-xs px-2.5 py-1 rounded-full border border-slate-850">
                      {t}
                    </span>
                  ))}
                </div>

                <div className="space-y-3 mt-6 text-xs sm:text-sm text-slate-300">
                  <div className="flex items-start space-x-2">
                    <RefreshCw className="h-4 w-4 text-amber-500 shrink-0 mt-0.5" />
                    <div>
                      <strong className="text-slate-200">The Cycles:</strong>
                      <p className="text-[11px] text-slate-400 mt-0.5">{el.generating}</p>
                      <p className="text-[11px] text-slate-400">{el.controlling}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start space-x-2">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-400 mt-0.5"><span className="text-red-400">If Strong:</span> {el.imbalanceStrong}</p>
                      <p className="text-[11px] text-slate-400"><span className="text-amber-400">If Weak:</span> {el.imbalanceWeak}</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6" id="cycles-explanation-cards">
          <div className="bg-slate-900 border border-slate-850 rounded-2xl p-6 sm:p-8">
            <h3 className="text-base font-serif text-slate-100 font-medium mb-4 flex items-center space-x-2">
            ...
            <div className="space-y-2 text-xs sm:text-sm">
              {GENERATING_CYCLE.map((cycle, index) => (
                <div key={index} className="flex justify-between items-center py-2 border-b border-slate-850 last:border-0">
                  ...
                  <span className="text-slate-500 italic text-[11px]">{cycle.action}</span>
