          ...
          {Object.values(GLOSSARY_DATA).map((item) => (
            <div key={item.term} className="bg-slate-900 border border-slate-850 rounded-xl p-6 hover:border-amber-500/20 transition-all space-y-3">
              <div className="flex justify-between items-start gap-2 flex-wrap">
                <h2 className="text-xl font-serif text-amber-500 font-medium tracking-wide">
                  {item.term}
                </h2>
                <span className="text-sm font-mono bg-slate-950 px-2 py-0.5 rounded border border-slate-850 text-slate-400">
                  {item.category}
                </span>
              </div>
              <p className="text-slate-300 text-base sm:text-lg leading-relaxed">{item.definition}</p>
              <div className="text-base text-slate-500 italic pt-1 border-t border-slate-950">
